...
export class Triangle {
  ...
  public setPoints(x1: number, y1: number, x2: number, y2: number, x3: number, y3: number): void {
    this.p1.set(x1, y1);
    this.p2.set(x2, y2);
    this.p3.set(x3, y3);
  }
  ...
  public contains(x: number, y: number): boolean {
    return this.containPoint(new Point(x, y));
  }

  private containPoint(p: Point): boolean {
